...
namespace bo
{
    ...
    void Node::evaluate()
    {
        ...
        for (auto it = this->dest_nodes.begin(); it != dest_nodes.end(); ++it)
        {
    ...
    {
        std::cout << "NodeOfDoubleOut::evaluate" << std::endl;
        Node::evaluate();
    }
    ...
    {
        std::cout << "NodeOfStringOut::evaluate" << std::endl;
        Node::evaluate();
    }
    ...
    void NodeOfSum::evaluate()
    {
        ...
        void *in_a_data = this->actual_props[1]->data;
        void *in_b_data = this->actual_props[2]->data;

        if (in_a_data != nullptr && in_b_data != nullptr)
        {

            void *main = this->main->data;

            double *m = static_cast<double *>(main);

            void *out = this->actual_props[0]->data;
    ...
    void NodeOfStringConcat::evaluate()
    {
        ...
        if (in_a_data != nullptr && in_b_data != nullptr)
        {
            ...
            std::string *m = static_cast<std::string *>(main);
    ...
    // template <typename T>
    // T DataHandler<T>::data(Property *prop)
    // {
    //     return static_cast<T>(prop->data);
    // }
